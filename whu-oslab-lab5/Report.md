...
> 本报告对 Lab5 的进程管理与调度实现进行说明，内容按照《操作系统内核实验报告规范》要求的结构补充了实验概述、系统设计、实验过程、测试验证以及问题与总结等部分，保留了原有描述并在需要处加以扩展。
