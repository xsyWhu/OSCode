...
static void run_priority_mlfq_demo(void)
{
    ...
    int worker_pids[PRIORITY_DEMO_WORKERS];
    ...
    int completion_idx[PRIORITY_DEMO_WORKERS];
    ...
    for (int i = 0; i < job_count; i++) {
        ...
        if (job_idx >= 0) {
            completion_idx[completion_count++] = job_idx;
            const struct priority_demo_job *job = &priority_demo_jobs[job_idx];
            printf("[PRIORITY-DEMO] worker %s pid=%d (priority=%d level=%d) done (status=%d) order=%d\n",
                   job->name, pid, job->priority, priority_to_level(job->priority), status, completion_count);
            klog(LOG_LEVEL_INFO,
                 "[PRIORITY-DEMO] worker %s pid=%d prio=%d lvl=%d done status=%d order=%d",
                 job->name, pid, job->priority, priority_to_level(job->priority), status, completion_count);
        } else {
            printf("[PRIORITY-DEMO] worker pid=%d done (status=%d) order=%d (unknown job)\n",
                   pid, status, completion_count + 1);
            klog(LOG_LEVEL_WARN, "[PRIORITY-DEMO] worker pid=%d done status=%d order=%d unmatched",
                 pid, status, completion_count + 1);
        }
    }

    printf("[PRIORITY-DEMO] === Completion Summary ===\n");
    klog(LOG_LEVEL_INFO, "[PRIORITY-DEMO] summary begin");
    int last_level = -1;
    int last_priority = PRIORITY_MAX + 1;
    int nondecreasing = 1;
    for (int i = 0; i < completion_count; i++) {
        const struct priority_demo_job *job = &priority_demo_jobs[completion_idx[i]];
        int level = priority_to_level(job->priority);
        printf("[PRIORITY-DEMO][SUMMARY] #%d -> %s (priority=%d initial_level=%d workload=%lu)\n",
               i + 1, job->name, job->priority, level, (unsigned long)job->workload);
        klog(LOG_LEVEL_INFO, "[PRIORITY-DEMO][SUMMARY] #%d %s prio=%d lvl=%d",
             i + 1, job->name, job->priority, level);
        if (last_level > level) {
            nondecreasing = 0;
        }
        if (last_priority < job->priority) {
            nondecreasing = 0;
        }
        if (level > last_level) {
            last_level = level;
        }
        if (job->priority < last_priority) {
            last_priority = job->priority;
        }
    }
    if (nondecreasing) {
        printf("[PRIORITY-DEMO][SUMMARY] completion order matches expected MLFQ behavior (high priority finished earlier).\n");
        klog(LOG_LEVEL_INFO, "[PRIORITY-DEMO][SUMMARY] order matches expectation");
    } else {
        printf("[PRIORITY-DEMO][SUMMARY] completion order deviates from ideal MLFQ expectation, inspect scheduler.\n");
        klog(LOG_LEVEL_WARN, "[PRIORITY-DEMO][SUMMARY] order deviates from expectation");
    }
}
